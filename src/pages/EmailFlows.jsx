...
const NODE_TYPES = {
  ...
  message: { label: 'E-mail', color: ACCENT, Icon: Mail },
};

function EmailPreview({ subject, body, isHtml, compact }) {
  return (
    <div className={`bg-slate-100 rounded-xl p-3 ${compact ? 'max-h-48' : ''} overflow-auto`}>
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
        <div className="px-3 py-2 border-b border-slate-100 bg-slate-50">
          <p className="text-[10px] text-slate-400 uppercase tracking-wider">Assunto</p>
          <p className="text-xs font-semibold text-slate-700 truncate">{subject || '(sem assunto)'}</p>
        </div>
        <div className="p-3 text-xs text-slate-700 leading-relaxed">
          {isHtml
            ? <div className="email-html-preview" dangerouslySetInnerHTML={{ __html: body ? sanitizeHtml(body) : '<span style="color:#94a3b8">(sem conteúdo)</span>' }} />
            : <div className="whitespace-pre-wrap">{body || '(sem conteúdo)'}</div>}
        </div>
      </div>
    </div>
  );
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              {flow.subject && (
                <p className="text-xs text-slate-500 mt-1.5 truncate"><span className="text-slate-400">Assunto:</span> {flow.subject}</p>
              )}
              {flow.description && (
        ...
        {showPreview && (
          ...
          >
            <div className="px-5 pb-5 pt-2 border-t border-slate-100 bg-slate-50/50">
              <EmailPreview subject={flow.subject} body={flow.message_template} isHtml={flow.is_html !== false} compact />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function VisualNode({ type, data, isLast, onUpdate, onRemove, canRemove, isDraggable, onAddAfter, isHtml }) {
          ...
          <div className="flex-1 min-w-0">
            ...
            {!expanded && (
              <p className="text-xs text-slate-500 truncate mt-0.5">
                {type === 'message' ? (data.message_template?.substring(0, 60) || 'Configure o conteúdo...') + (data.message_template?.length > 60 ? '...' : '') : ''}
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder={isHtml ? '<p>Olá {{nome}}, ...</p>' : 'Digite o conteúdo do e-mail...'}
                        rows={7}
                        className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#2e6299]/30 resize-none font-mono leading-relaxed"
                      />
                    </div>
                    <div>
                        ...
                        {TEMPLATE_VARS.map(v => {
                          ...
                          return (
                            <button
                              ...
                              className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-mono bg-slate-100 text-slate-600 hover:bg-[#2e6299]/10 hover:text-[#2e6299] transition-all"
                              title={v.desc}
                            >
                    ...
                    {data.message_template && (
                      <div>
                        ...
                        <EmailPreview body={data.message_template} isHtml={isHtml} compact />
                      </div>
                    )}
                  </div>
                )}
      ...
      <div className="relative bg-slate-50 w-full max-w-4xl mx-auto my-4 rounded-2xl shadow-2xl overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#2e6299] to-[#1e4470] flex items-center justify-center shadow-lg shadow-blue-500/20">
        ...
        <div className="flex-1 overflow-auto">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-0 min-h-full">
            <div className="lg:col-span-2 bg-white border-r border-slate-200 p-5 space-y-4 overflow-auto">
              ...
              <div>
                <label className="text-[11px] font-semibold text-slate-500 mb-1.5 block uppercase tracking-wider">Assunto do E-mail</label>
                <Input value={form.subject} onChange={e => setForm(f => ({ ...f, subject: e.target.value }))} placeholder="Ex: Sua reserva foi confirmada!" className="h-10 rounded-xl" />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-semibold text-slate-500 mb-1.5 block uppercase tracking-wider">Formato</label>
                  <button
                    onClick={() => setForm(f => ({ ...f, is_html: !f.is_html }))}
                    className={`inline-flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium transition-all w-full justify-center ${form.is_html ? 'bg-[#2e6299]/10 text-[#2e6299] border border-[#2e6299]/30' : 'bg-slate-50 text-slate-500 border border-slate-200'}`}
                  >
                    {form.is_html ? <Code className="w-4 h-4" /> : <Type className="w-4 h-4" />}
                    {form.is_html ? 'HTML' : 'Texto'}
                  </button>
                </div>
                <div>
                  ...
                  <button
                    ...
                    className={`inline-flex items-center gap-2 px-3 py-2.5 rounded-xl text-sm font-medium transition-all w-full justify-center ${form.enabled ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-50 text-slate-500 border border-slate-200'}`}
                  >
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  {nodes.filter(n => n.type === 'message').length < 5 && (
                    <button
                      ...
                      className="flex items-center gap-2 p-3 rounded-xl bg-[#2e6299]/10 text-[#2e6299] hover:bg-[#2e6299]/20 transition-all text-xs font-medium"
                    >
              ...
              {triggerInfo && (
                <div className="border-t border-slate-100 pt-4">
                  ...
                  <div className="bg-slate-50 rounded-xl p-3 space-y-2">
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      <Mail className="w-3.5 h-3.5 text-[#2e6299]" />
                      <span className="text-slate-600">
                        Enviar: <strong>{nodes.filter(n => n.type === 'message').length} e-mail(s)</strong>
                      </span>
                    </div>
                  </div>
                </div>
        ...
        <div className="bg-white border-t border-slate-200 px-6 py-4 flex justify-between items-center flex-shrink-0">
          ...
          <div className="flex gap-3">
            ...
            <Button
              onClick={handleSave}
              disabled={saving || !form.name || !form.subject || !nodes.some(n => n.type === 'message' && n.data.message_template)}
              className="rounded-xl text-white shadow-lg shadow-blue-500/20"
              style={{ backgroundColor: ACCENT }}
            >
...
function TestModal({ flow, onClose, onSend, sending }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await fetch(`/api/email/test/${testId}`);
  ...
  const handleSend = async () => {
    const result = await onSend(recipient, flow);
    if (result?.testId) {
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          {!testState ? (
            <>
              <div>
                <label className="text-xs font-semibold text-slate-600 mb-1.5 block">E-mail do destinatário</label>
                <Input value={recipient} onChange={e => setRecipient(e.target.value)} placeholder="cliente@exemplo.com" className="h-10 rounded-xl" />
              </div>
                    ...
                    {flowNodes.map((node, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs text-blue-600">
                        {node.type === 'delay' ? (
                          ...
                          <><Mail className="w-3.5 h-3.5 text-[#2e6299]" /><span className="truncate">E-mail: {(node.data.message_template || '').substring(0, 40)}...</span></>
                        )}
                      </div>
                    ))}
                  </div>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-50 rounded-xl p-3">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-slate-500">{testState.sent}/{testState.total} enviado(s)</span>
                </div>
                ...
                <div className="space-y-1.5">
                  {testState.schedule.map((step, i) => (
                    <div key={i} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs ${step.status === 'sent' ? 'bg-emerald-50' : step.status === 'sending' || step.status === 'delaying' ? 'bg-blue-50' : step.status === 'error' ? 'bg-red-50' : 'bg-slate-50'}`}>
                      ...
                      <span className="font-medium text-slate-700">E-mail {step.step}</span>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-3">
          ...
          {!testState && (
            <Button onClick={handleSend} disabled={!recipient || sending} className="rounded-xl text-white" style={{ backgroundColor: ACCENT }}>
  ...
  return (
    <div className={`rounded-xl border transition-colors ${isSent ? 'bg-slate-50 border-slate-100' : 'bg-red-50/60 border-red-100'}`}>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-medium text-slate-700 truncate">{log.recipient_email || '—'}</p>
          <p className="text-[10px] text-slate-400 truncate">
            {isSent ? (log.subject || log.message?.substring(0, 80) || 'Sem assunto') : (log.error_message?.substring(0, 80) || 'Erro sem detalhes')}
          </p>
        </div>
        ...
        {expanded && (
          ...
          >
            <div className="px-3 pb-3 pt-1 space-y-2 text-xs">
              <div className="grid grid-cols-2 gap-2">
                <div><span className="text-[10px] text-slate-400 uppercase tracking-wider">Destinatário</span><p className="text-slate-700 font-medium break-all">{log.recipient_email || '—'}</p></div>
              ...
              {log.subject && (
                <div><span className="text-[10px] text-slate-400 uppercase tracking-wider">Assunto</span><p className="text-slate-700 font-medium">{log.subject}</p></div>
              )}
              {log.message && (
                <div>
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider">Conteúdo</span>
                  <div className="mt-1 bg-white rounded-lg border border-slate-200 p-2.5 text-slate-800 leading-relaxed max-h-40 overflow-auto">
                    <div dangerouslySetInnerHTML={{ __html: sanitizeHtml(log.message) }} />
                  </div>
                </div>
              )}
              {!isSent && (
...
function LogsModal({ flow, onClose }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/email/logs?flow_id=${flow.id}&limit=200`)
      .then(r => r.json())
  ...
  const handleExport = () => {
    ...
    const headers = ['Data/Hora', 'Destinatário', 'Status', 'Assunto', 'Conteúdo', 'Erro'];
    ...
    a.download = `logs_email_${safeName}_${format(new Date(), 'yyyyMMdd_HHmm')}.csv`;
    a.click();
        ...
        <div className="flex-1 overflow-auto p-4">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-slate-400">
              ...
              <p className="text-sm">{filter === 'error' ? 'Nenhum erro registrado' : 'Nenhum envio registrado'}</p>
            </div>
          ) : (
...
function ConfigModal({ onClose, onSaved }) {
  const [config, setConfig] = useState({ host: '', port: 587, secure: false, user: '', pass: '', from_name: '', from_email: '' });
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    fetch('/api/email/config')
      .then(r => r.json())
      .then(d => { if (d.ok) setConfig(c => ({ ...c, ...d.data })); setLoading(false); })
      .catch(() => setLoading(false));
  }, []);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-slate-800">Configurar E-mail (SMTP)</h3>
            <p className="text-xs text-slate-400">Servidor de envio de e-mails</p>
          </div>
        </div>
        <div className="p-6 space-y-4">
          {loading ? (
            <div className="flex items-center justify-center py-8"><Loader2 className="w-6 h-6 text-slate-400 animate-spin" /></div>
          ) : (
            <>
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-2">
                  <label className="text-xs font-semibold text-slate-600 mb-1.5 block">Servidor SMTP (host)</label>
                  <Input value={config.host} onChange={e => setConfig(c => ({ ...c, host: e.target.value }))} placeholder="smtp.gmail.com" className="h-10 rounded-xl" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-600 mb-1.5 block">Porta</label>
                  <Input type="number" value={config.port} onChange={e => setConfig(c => ({ ...c, port: e.target.value }))} placeholder="587" className="h-10 rounded-xl" />
                </div>
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-600 mb-1.5 block">Usuário</label>
                <Input value={config.user} onChange={e => setConfig(c => ({ ...c, user: e.target.value }))} placeholder="usuario@dominio.com" className="h-10 rounded-xl" />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-600 mb-1.5 block">Senha</label>
                <Input type="password" value={config.pass} onChange={e => setConfig(c => ({ ...c, pass: e.target.value }))} placeholder="••••••" className="h-10 rounded-xl" />
                <p className="text-[10px] text-slate-400 mt-1">Deixe o valor mascarado para manter a senha atual.</p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-slate-600 mb-1.5 block">Nome do remetente</label>
                  <Input value={config.from_name} onChange={e => setConfig(c => ({ ...c, from_name: e.target.value }))} placeholder="UNYCO" className="h-10 rounded-xl" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-600 mb-1.5 block">E-mail remetente</label>
                  <Input value={config.from_email} onChange={e => setConfig(c => ({ ...c, from_email: e.target.value }))} placeholder="no-reply@unyco.com" className="h-10 rounded-xl" />
                </div>
              </div>
              <button
                onClick={() => setConfig(c => ({ ...c, secure: !c.secure }))}
                className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${config.secure ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-50 text-slate-500 border border-slate-200'}`}
              >
                {config.secure ? <ToggleRight className="w-5 h-5" /> : <ToggleLeft className="w-5 h-5" />}
                Conexão segura (SSL/TLS) {config.secure ? 'ativada' : 'desativada'}
              </button>
              {error && (
                <p className="text-xs text-red-500 bg-red-50 border border-red-100 rounded-lg p-2.5">{error}</p>
              )}
            </>
          )}
        </div>
        <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-3">
          <Button onClick={onClose} variant="outline" className="rounded-xl">Cancelar</Button>
          <Button onClick={handleSave} disabled={saving || loading} className="rounded-xl text-white" style={{ backgroundColor: ACCENT }}>
            {saving ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <CheckCircle2 className="w-4 h-4 mr-2" />}
            Salvar
          </Button>
        </div>
...
export default function EmailFlows() {
  ...
  const handleSave = async (formData) => {
    setSaving(true);
    try {
      const url = editorFlow ? `/api/email/flows/${editorFlow.id}` : '/api/email/flows';
      const method = editorFlow ? 'PUT' : 'POST';
      ...
      const data = await res.json();
      if (data.ok) {
        showToast(editorFlow ? 'Fluxo atualizado!' : 'Fluxo criado!');
        setShowEditor(false);
        setEditorFlow(null);
        await Promise.all([loadFlows(), loadStats()]);
      } else {
  ...
  const handleToggle = async (flow) => {
    try {
      const res = await fetch(`/api/email/flows/${flow.id}/toggle`, { method: 'PATCH' });
      const data = await res.json();
      if (data.ok) {
        await Promise.all([loadFlows(), loadStats()]);
      }
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/email/flows/${deleteFlow.id}`, { method: 'DELETE' });
      const data = await res.json();
      if (data.ok) {
  ...
  const handleTest = async (recipient, flow) => {
    setSending(true);
    try {
      const res = await fetch('/api/email/test', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ recipient, flow_id: flow.id }),
      });
      const data = await res.json();
      if (data.ok) {
        if (data.testId) {
  ...
  const filtered = useMemo(() => {
    return flows.filter(f => {
      ...
      const matchEvent = filterEvent === 'all' || f.trigger_event === filterEvent;
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <div className="flex-1">
          <PageHeader title="Automacoes de E-mail" description="Gerencie fluxos de envio automatico por e-mail" icon={Mail} />
        </div>
        <Button onClick={() => setShowConfig(true)} variant="outline" className="rounded-xl border-slate-300">
          <Settings2 className="w-4 h-4 mr-2" />
          Configurar E-mail
        </Button>
        <Button onClick={() => { setEditorFlow(null); setShowEditor(true); }} className="rounded-xl text-white shadow-lg shadow-blue-500/20" style={{ backgroundColor: ACCENT }}>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Fluxos Ativos', value: stats.flows?.active || 0, total: stats.flows?.total || 0, IconComp: Zap, color: ACCENT, bg: 'bg-blue-50' },
            { label: 'Total Enviados', value: stats.messages?.total_sent || 0, IconComp: Send, color: '#10b981', bg: 'bg-emerald-50' },
      ...
      {loading ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-slate-200 shadow-sm">
          <Loader2 className="w-8 h-8 text-[#2e6299] mx-auto animate-spin" />
          <p className="text-slate-500 mt-3">Carregando fluxos...</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="bg-white rounded-2xl p-12 text-center border border-slate-200 shadow-sm">
          <div className="w-16 h-16 bg-blue-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Mail className="w-8 h-8 text-[#2e6299]/40" />
          </div>
          <h3 className="font-semibold text-slate-700 mb-1">Nenhum fluxo encontrado</h3>
          <p className="text-sm text-slate-400">Crie seu primeiro fluxo de automacao de e-mail</p>
        </div>
      ) : (
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
            className={`fixed bottom-6 right-6 z-[60] px-5 py-3 rounded-xl shadow-lg text-sm font-medium flex items-center gap-2 ${toast.type === 'error' ? 'bg-red-500 text-white' : 'bg-[#2e6299] text-white'}`}
          >
      ...
      {showConfig && (
        <ConfigModal onClose={() => setShowConfig(false)} onSaved={() => showToast('Configuração salva!')} />
      )}
