...
export default function CentralAPIs() {
  ...
  const isCoobmais = (api) => api?.name === 'Coobmais';

  const fetchCoobTokenInfo = useCallback(async () => {
    setLoadingTokenInfo(true);
    try {
      const res = await fetch('/api/central/coobmais/token');
      const data = await res.json();
      if (data.success) setCoobTokenInfo(data);
      else setCoobTokenInfo({ error: data.error });
    } catch (err) {
      setCoobTokenInfo({ error: err.message });
    } finally {
      setLoadingTokenInfo(false);
    }
  }, []);

  const handleRefreshCoobToken = async () => {
    setRefreshingToken(true);
    try {
      const res = await fetch('/api/central/coobmais/refresh-token', { method: 'POST' });
      const data = await res.json();
      if (data.success) {
        setCoobTokenInfo({ ...data, hasCredentials: true });
      } else {
        setCoobTokenInfo({ error: data.error });
      }
    } catch (err) {
      setCoobTokenInfo({ error: err.message });
    } finally {
  ...
  const handleEditApi = (api) => {
    ...
    setEditingApi(api);
    if (isCoobmais(api)) fetchCoobTokenInfo();
  };
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (isCoobmais(editingApi) && (editForm.accessKey || editForm.password)) {
          await fetchCoobTokenInfo();
        }
        setEditingApi(null);
        await fetchApis();
      }
          ...
          >
            ...
            >
              ...
              <div className="p-6 space-y-4">
                ...
                {isCoobmais(editingApi) ? (
                <>
                <div>
                  <label className="text-sm font-semibold text-slate-700 mb-2 block">URL de Autenticação</label>
                  <input
                    type="text"
                    value={editForm.authUrl}
                    onChange={(e) => setEditForm(prev => ({ ...prev, authUrl: e.target.value }))}
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400"
                    placeholder="https://apiprod.coobmais.com.br/auth/api/Users/Authenticate"
                  />
                  <p className="text-xs text-slate-400 mt-1.5">Endpoint de login que devolve o JWT</p>
                </div>
                <div>
                  <label className="text-sm font-semibold text-slate-700 mb-2 block">AccessKey</label>
                  <input
                    type="text"
                    value={editForm.accessKey}
                    onChange={(e) => setEditForm(prev => ({ ...prev, accessKey: e.target.value }))}
                    className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400"
                    placeholder={apiConfig?.Coobmais?.accessKey || 'Cole a AccessKey fornecida pela Coobmais'}
                  />
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder={apiConfig?.Coobmais?.password || 'Senha da AccessKey'}
                    />
                    <button
                      type="button"
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold text-slate-700">Token JWT (auto-gerado)</span>
                    </div>
                    <button
                      type="button"
                      onClick={handleRefreshCoobToken}
                      disabled={refreshingToken || !coobTokenInfo?.hasCredentials}
                      ...
                      title={!coobTokenInfo?.hasCredentials ? 'Configure AccessKey e senha primeiro' : 'Forçar regeneração do token'}
                    >
                  ...
                  {loadingTokenInfo ? (
                    <div className="flex items-center gap-2 text-xs text-slate-500">
                      <Loader2 className="w-3.5 h-3.5 animate-spin" /> Carregando token...
                    </div>
                  ) : coobTokenInfo?.error ? (
                    <div className="flex items-start gap-2 text-xs text-red-600">
                      <AlertTriangle className="w-3.5 h-3.5 mt-0.5 flex-shrink-0" />
                      <span>{coobTokenInfo.error}</span>
                    </div>
                  ) : coobTokenInfo?.tokenFull ? (
                    <>
                      <div className="relative">
                        <textarea
                          readOnly
                          value={showFullToken ? coobTokenInfo.tokenFull : coobTokenInfo.tokenPreview}
                          rows={showFullToken ? 4 : 2}
                          className="w-full px-3 py-2 pr-10 rounded-lg border border-slate-200 bg-white text-xs font-mono text-slate-700 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                        />
                        <button
                          type="button"
                          onClick={() => setShowFullToken(!showFullToken)}
                          className="absolute right-2 top-2 p-1 hover:bg-slate-100 rounded transition-colors"
                          title={showFullToken ? 'Ocultar' : 'Ver completo'}
                        >
                          {showFullToken ? <EyeOff className="w-3.5 h-3.5 text-slate-400" /> : <Eye className="w-3.5 h-3.5 text-slate-400" />}
                        </button>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-slate-500">Expira em:</span>
                        <span className="font-semibold text-slate-700">
                          {coobTokenInfo.expiresAt ? new Date(coobTokenInfo.expiresAt).toLocaleString('pt-BR') : '--'}
                          {coobTokenInfo.expiresInSeconds != null && (
                            <span className="ml-2 text-emerald-600">
                              ({Math.floor(coobTokenInfo.expiresInSeconds / 86400)}d {Math.floor((coobTokenInfo.expiresInSeconds % 86400) / 3600)}h)
                            </span>
                          )}
                        </span>
                      </div>
                      <button
                        type="button"
                        onClick={() => navigator.clipboard?.writeText(coobTokenInfo.tokenFull)}
                    ...
                    <p className="text-xs text-slate-500">
                      Configure AccessKey e senha acima e salve para gerar o token automaticamente.
                    </p>
                  )}
                </div>
                </>
