...
import { Calendar, Building2, DollarSign, TrendingUp, RefreshCw, Filter, ChevronDown, ChevronUp, Loader2, AlertCircle, MapPin, BarChart3, Activity, FileDown, CheckCircle2, XCircle, Clock, CreditCard, Users, Maximize2, X, Download } from 'lucide-react';
...
import { format, subDays, subMonths, startOfMonth, endOfMonth, isWithinInterval, startOfDay, endOfDay, differenceInCalendarDays } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { BarChart, Bar, PieChart, Pie, Cell, ResponsiveContainer, XAxis, YAxis, Tooltip, Legend, ComposedChart, Line } from 'recharts';
...
function parseCityName(city) {
  if (!city) return 'N/I';
  if (city.startsWith('{')) { try { return JSON.parse(city).name || 'N/I'; } catch { return 'N/I'; } }
  return city;
}
...
function BrazilMap({ data, onMaximize }) {
  const [hovered, setHovered] = useState(null);

  const maxValue = Math.max(...data.map(d => d.reservas), 1);

  function getStateColor(uf) {
    const d = dataMap[uf];
    if (!d) return '#eef2f7';
    const intensity = Math.min(d.reservas / maxValue, 1);
    ...
    <div className="relative" style={{ minHeight: '300px' }}>
      <svg viewBox="0 15 480 460" className="w-full h-auto" style={{ minHeight: '300px' }} preserveAspectRatio="xMidYMid meet">
        <defs>
          ...
          <filter id="tooltipShadow" x="-15%" y="-15%" width="130%" height="130%">
          ...
          {Object.entries(BRAZIL_STATES_PATHS).map(([uf, pathD]) => {
            ...
            return (
              <path
                key={uf}
                d={pathD}
                fill={d ? getStateColor(uf) : 'url(#stateGrad)'}
        ...
        {Object.entries(ESTADOS_CENTERS).map(([uf, pos]) => {
          ...
          const radius = Math.max(12, Math.min(26, 12 + (d.reservas / maxValue) * 14));
          return (
            <g key={uf}
              onMouseEnter={() => setHovered(uf)}
              onMouseLeave={() => setHovered(null)}
            ...
            >
              ...
              />
              <circle cx={pos.x} cy={pos.y} r={radius} fill="url(#bubbleGrad)" className="pointer-events-none" />
              ...
              {isHovered && (
                <g className="pointer-events-none">
                  <rect x={pos.x > 300 ? pos.x - 165 : pos.x + 20} y={pos.y - 42} width="148" height="56" rx="8" fill="white" stroke="#e2e8f0" strokeWidth="0.8" filter="url(#tooltipShadow)" />
                  <text x={pos.x > 300 ? pos.x - 157 : pos.x + 28} y={pos.y - 24} fontSize="10" fontWeight="700" fill="#1e293b">{pos.name}</text>
                  <text x={pos.x > 300 ? pos.x - 157 : pos.x + 28} y={pos.y - 11} fontSize="8.5" fill="#64748b">{d.reservas} reserva{d.reservas !== 1 ? 's' : ''}</text>
                  <text x={pos.x > 300 ? pos.x - 157 : pos.x + 28} y={pos.y + 3} fontSize="9.5" fill="#2e6299" fontWeight="700">{formatCurrency(d.receita)}</text>
                </g>
              )}
            </g>
          );
      ...
      {onMaximize && (
        <button
          onClick={onMaximize}
          className="absolute top-2 right-2 p-2 bg-white/90 hover:bg-white rounded-lg shadow-sm border border-slate-200 text-slate-500 hover:text-[#2e6299] transition-all"
          title="Maximizar"
        >
...
function MapModal({ data, onClose }) {
  const maxValue = Math.max(...data.map(d => d.reservas), 1);
  const sorted = [...data].sort((a, b) => b.reservas - a.reservas);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-auto" onClick={e => e.stopPropagation()}>
        <div className="sticky top-0 bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between z-10 rounded-t-2xl">
          <h3 className="font-bold text-[#2e6299] text-lg flex items-center gap-2">
            <MapPin className="w-5 h-5" />
            Mapa de Reservas por Estado
          </h3>
              ...
              {sorted.map((item, idx) => (
                <div key={item.estado} className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 hover:bg-slate-100 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400">{item.reservas} reserva{item.reservas !== 1 ? 's' : ''}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-bold text-emerald-600">{formatCurrency(item.receita)}</p>
                    <div className="w-16 h-1.5 bg-slate-200 rounded-full mt-1">
                      <div className="h-full bg-[#2e6299] rounded-full" style={{ width: `${(item.reservas / maxValue) * 100}%` }} />
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="pt-4 border-t border-slate-200">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[#2e6299]/5 rounded-xl p-3 text-center">
                  ...
                  <p className="text-xl font-bold text-[#2e6299]">{data.reduce((s, d) => s + d.reservas, 0)}</p>
                </div>
...
export default function DashboardReservas() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (!bData.ok) throw new Error(bData.error || 'Erro ao carregar reservas');
      setBookings(bData.data || []);
      setPayments(pData.ok ? pData.data : []);
    } catch (err) {
      setError(err);
    } finally {
      setLoading(false);
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.save(`reservas_receita_${new Date().toISOString().split('T')[0]}.pdf`);
    } catch (err) {
  ...
  const handleExportCSV = () => {
    const headers = ['Localizador', 'Hospede', 'CPF', 'Email', 'Hotel', 'Cidade', 'Check-in', 'Check-out', 'Adultos', 'Criancas', 'Valor (R$)', 'Status', 'Pagamento', 'Metodo Pagamento', 'Criado em'];
    const rows = filtered.map(b => [
      ...
      b.user_email || '',
      b.hotel_name || '',
      parseCityName(b.hotel_city),
      b.check_in ? format(new Date(b.check_in), 'dd/MM/yyyy') : '',
      b.check_out ? format(new Date(b.check_out), 'dd/MM/yyyy') : '',
      b.adults || 1,
      b.children || 0,
      parseFloat(b.total_price || 0).toFixed(2).replace('.', ','),
      STATUS_LABELS[b.status] || b.status || '',
      PAYMENT_STATUS_LABELS[b.payment_status] || b.payment_status || 'Sem pagamento',
  ...
  const filtered = useMemo(() => {
    return bookings.filter(b => {
      ...
      if (dateFilter === 'mes_atual') {
        if (bDate < startOfMonth(new Date())) return false;
      } else if (dateFilter === 'personalizado') {
        if (customStartDate && customEndDate) {
          if (!isWithinInterval(bDate, { start: startOfDay(customStartDate), end: endOfDay(customEndDate) })) return false;
        }
      } else if (dateFilter !== 'todos') {
        const days = parseInt(dateFilter);
        if (!isNaN(days)) {
          if (bDate < subDays(new Date(), days)) return false;
        }
      }
      if (statusFilter !== 'todos' && b.status !== statusFilter) return false;
      return true;
    });
  }, [bookings, dateFilter, customStartDate, customEndDate, statusFilter]);

  const getFilterLabel = () => {
    if (dateFilter === 'mes_atual') return 'Mes atual';
    if (dateFilter === 'personalizado') return 'Personalizado';
    if (dateFilter === 'todos') return 'Todos os periodos';
  ...
  const stats = useMemo(() => {
    ...
    const receitaTotal = filtered.reduce((s, b) => s + parseFloat(b.total_price || 0), 0);

    const paidFiltered = filtered.filter(b => b.payment_status === 'paid');
    const pendingFiltered = filtered.filter(b => b.payment_status === 'pending');
    const receitaRecebida = paidFiltered.reduce((s, b) => s + parseFloat(b.payment_amount || b.total_price || 0), 0);
    const receitaPendente = pendingFiltered.reduce((s, b) => s + parseFloat(b.payment_amount || b.total_price || 0), 0);

    const mesAtual = bookings.filter(b => new Date(b.created_at) >= startOfMonth(new Date())).length;
    const mesPassado = bookings.filter(b => {
      const d = new Date(b.created_at);
      return d >= startOfMonth(subMonths(new Date(), 1)) && d < startOfMonth(new Date());
    }).length;
    const crescimento = mesPassado > 0 ? Math.round(((mesAtual - mesPassado) / mesPassado) * 100) : mesAtual > 0 ? 100 : 0;

    return { total, confirmed, completed, cancelled, receitaTotal, receitaRecebida, receitaPendente, crescimento };
  }, [filtered, bookings]);

  const statusChartData = useMemo(() => {
    const colors = { confirmed: '#10b981', completed: '#3b82f6', cancelled: '#ef4444' };
    return [
      { name: 'Confirmadas', value: stats.confirmed, color: colors.confirmed },
      { name: 'Concluidas', value: stats.completed, color: colors.completed },
  ...
  const hotelRanking = useMemo(() => {
    const map = {};
    filtered.forEach(b => {
      const name = b.hotel_name || 'Desconhecido';
      if (!map[name]) map[name] = { hotel: name, reservas: 0, receita: 0 };
      map[name].reservas++;
      map[name].receita += parseFloat(b.total_price || 0);
    });
    return Object.values(map).sort((a, b) => b.reservas - a.reservas).slice(0, 8);
  }, [filtered]);

  const stateMapData = useMemo(() => {
    const map = {};
    filtered.forEach(b => {
      const state = (b.hotel_state || '').toUpperCase().trim();
      if (!state) return;
      if (!map[state]) map[state] = { estado: state, reservas: 0, receita: 0 };
      map[state].reservas++;
      map[state].receita += parseFloat(b.total_price || 0);
    });
  ...
  const receitaPorMes = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      data.push({
        ...
        receita: reservasMes.reduce((s, b) => s + parseFloat(b.total_price || 0), 0)
      });
    }
    return data;
  }, [bookings]);
  ...
  const comparativoMensal = useMemo(() => {
    ...
    const reservasAtual = bookings.filter(b => new Date(b.created_at) >= startOfMonth(mesAtual));
    const reservasPassado = bookings.filter(b => {
      const d = new Date(b.created_at);
      return d >= startOfMonth(mesPassado) && d < startOfMonth(mesAtual);
    });
    return [
      { nome: format(mesPassado, 'MMMM', { locale: ptBR }), novas: reservasPassado.length, receita: reservasPassado.reduce((s, b) => s + parseFloat(b.total_price || 0), 0) },
      { nome: format(mesAtual, 'MMMM', { locale: ptBR }), novas: reservasAtual.length, receita: reservasAtual.reduce((s, b) => s + parseFloat(b.total_price || 0), 0) }
    ];
  }, [bookings]);

  const paymentMethodData = useMemo(() => {
    const colors = ['#2e6299', '#10b981', '#f59e0b', '#8b5cf6'];
    const map = {};
    filtered.forEach(b => {
      if (!b.payment_method) return;
      const method = PAYMENT_METHOD_LABELS[b.payment_method] || b.payment_method;
      if (!map[method]) map[method] = { name: method, value: 0, amount: 0 };
      map[method].value++;
      map[method].amount += parseFloat(b.payment_amount || b.total_price || 0);
    });
    return Object.values(map).map((m, idx) => ({ ...m, color: colors[idx % colors.length] }));
  }, [filtered]);

  const paymentStatusData = useMemo(() => {
    const withPayment = filtered.filter(b => b.payment_status);
    const withoutPayment = filtered.filter(b => !b.payment_status);
    const paid = withPayment.filter(b => b.payment_status === 'paid');
    const pending = withPayment.filter(b => b.payment_status === 'pending');
    const canceled = withPayment.filter(b => b.payment_status === 'canceled');
    return [
      { name: 'Pago', value: paid.length, color: '#10b981' },
      { name: 'Pendente', value: pending.length, color: '#f59e0b' },
      { name: 'Cancelado', value: canceled.length, color: '#ef4444' },
      { name: 'Sem Pagamento', value: withoutPayment.length, color: '#cbd5e1' },
    ].filter(d => d.value > 0);
  }, [filtered]);
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <PageHeader title="Reservas & Receita" description="Visao geral de reservas e faturamento" icon={BarChart3} />
        <div className="bg-red-50 border border-red-200 rounded-2xl p-8 text-center">
          <AlertCircle className="w-12 h-12 text-red-500 mx-auto mb-4" />
          <h3 className="text-lg font-semibold text-red-700">Erro ao carregar dados</h3>
          <p className="text-red-600 mt-2">{error.message}</p>
          <Button onClick={fetchData} className="mt-4 bg-red-600 hover:bg-red-700">Tentar novamente</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-slate-500">
                  {statusFilter !== 'todos' ? `Status: ${STATUS_LABELS[statusFilter] || statusFilter}` : 'Todos os status'}
              ...
              >
                <div className="px-6 pb-6 border-t border-slate-100">
                        ...
                        <div>
                          ...
                          <Popover>
                            ...
                            <PopoverContent className="w-auto p-0" align="start">
                              <CalendarComponent mode="single" selected={customStartDate} onSelect={setCustomStartDate} locale={ptBR} />
                            </PopoverContent>
                          </Popover>
                        </div>
                        <div>
                          ...
                          <Popover>
                            ...
                            <PopoverContent className="w-auto p-0" align="start">
                              <CalendarComponent mode="single" selected={customEndDate} onSelect={setCustomEndDate} locale={ptBR} disabled={(date) => customStartDate ? date < customStartDate : false} />
                            </PopoverContent>
                          </Popover>
                        </div>
                      </>
                  ...
                  <div className="mt-4 flex gap-2">
                    <Button
                      onClick={() => { setDateFilter('mes_atual'); setCustomStartDate(null); setCustomEndDate(null); setStatusFilter('todos'); }}
                      variant="outline"
                      size="sm"
                    ...
                    <Button onClick={fetchData} disabled={loading} size="sm" className="rounded-xl h-9 bg-[#2e6299] hover:bg-[#2e6299]/90">
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
              <ModernStatsCard title="Total Reservas" value={stats.total} icon={Calendar} color="blue" trend={stats.crescimento} />
              <ModernStatsCard
                title="Confirmadas"
                value={stats.confirmed}
                icon={CheckCircle2}
                subtitle={stats.total > 0 ? `${Math.round((stats.confirmed / stats.total) * 100)}% do total` : '0% do total'}
                showCircle={true}
                circleValue={stats.total > 0 ? Math.round((stats.confirmed / stats.total) * 100) : 0}
              />
              <ModernStatsCard title="Receita Total" value={formatCurrency(stats.receitaTotal)} icon={DollarSign} color="green" subtitle="soma das reservas" />
              <ModernStatsCard title="Receita Recebida" value={formatCurrency(stats.receitaRecebida)} icon={TrendingUp} color="purple" subtitle="pagamentos confirmados" />
              <ModernStatsCard
                title="Crescimento"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                <h3 className="font-bold text-[#2e6299] mb-6 flex items-center gap-2">
                  ...
                  Receita x Reservas por Mes
                </h3>
                <ResponsiveContainer width="100%" height={280}>
                  <ComposedChart data={receitaPorMes}>
                    <XAxis dataKey="mes" stroke="#94a3b8" style={{ fontSize: '12px' }} />
                    <YAxis yAxisId="left" stroke="#94a3b8" style={{ fontSize: '12px' }} allowDecimals={false} />
                    <YAxis yAxisId="right" orientation="right" stroke="#94a3b8" style={{ fontSize: '12px' }} tickFormatter={(v) => v >= 1000 ? `R$ ${(v/1000).toFixed(0)}k` : `R$ ${v}`} />
                    <Tooltip contentStyle={tooltipStyle} formatter={(value, name) => [name === 'receita' ? formatCurrency(value) : value, name === 'receita' ? 'Receita' : 'Reservas']} />
                    <Legend />
                    <Bar yAxisId="left" dataKey="reservas" name="Reservas" fill="#2e6299" radius={[4, 4, 0, 0]} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                ...
                {stateMapData.length > 0 ? (
                  <BrazilMap data={stateMapData} onMaximize={() => setShowMapModal(true)} />
                ) : (
                  <div className="flex items-center justify-center h-[280px] text-sm text-slate-400">Sem dados de localidade</div>
                )}
              </div>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                <h3 className="font-bold text-[#2e6299] mb-6 flex items-center gap-2">
                  ...
                  Hoteis mais Reservados
                </h3>
                {hotelRanking.length > 0 ? (
                  <ResponsiveContainer width="100%" height={280}>
                    <BarChart data={hotelRanking} layout="vertical">
                      <XAxis type="number" stroke="#94a3b8" style={{ fontSize: '12px' }} allowDecimals={false} />
                      <YAxis dataKey="hotel" type="category" stroke="#94a3b8" style={{ fontSize: '10px' }} width={120} />
                      <Tooltip contentStyle={tooltipStyle} formatter={(value, name) => [name === 'receita' ? formatCurrency(value) : `${value} reservas`, name === 'receita' ? 'Receita' : 'Reservas']} />
                      <Bar dataKey="reservas" fill="#2e6299" radius={[0, 8, 8, 0]} name="Reservas" />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
                  <div className="flex items-center justify-center h-[280px] text-sm text-slate-400">Sem dados</div>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div>
                    <h4 className="text-xs font-medium text-slate-500 mb-3 uppercase tracking-wider">Status</h4>
                  ...
                  <div>
                    <h4 className="text-xs font-medium text-slate-500 mb-3 uppercase tracking-wider">Metodos</h4>
                ...
                {paymentStatusData.length > 0 && (
                  <ResponsiveContainer width="100%" height={160}>
                    <PieChart>
                      <Pie data={paymentStatusData} cx="50%" cy="50%" innerRadius={40} outerRadius={65} paddingAngle={3} dataKey="value">
                  ...
                  {comparativoMensal.map((mes, idx) => (
                    <div key={idx} className={`p-4 rounded-xl ${idx === 1 ? 'bg-[#2e6299]/5 border border-[#2e6299]/20' : 'bg-slate-50'}`}>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <p className="text-xs text-slate-500">Novas Reservas</p>
                          <p className="text-2xl font-bold text-[#2e6299]">{mes.novas}</p>
                        </div>
                        <div>
                          <p className="text-xs text-slate-500">Receita</p>
                          <p className="text-2xl font-bold text-emerald-600">{formatCurrency(mes.receita)}</p>
                        </div>
                      </div>
                    </div>
                  ))}
                  {comparativoMensal.length === 2 && (
                    <div className="pt-4 border-t border-slate-200">
                      <div className="grid grid-cols-2 gap-4 text-center">
                        <div>
                          <p className="text-xs text-slate-500">Variacao Reservas</p>
                          <p className={`text-lg font-bold ${comparativoMensal[1].novas >= comparativoMensal[0].novas ? 'text-emerald-600' : 'text-red-600'}`}>
                            {comparativoMensal[0].novas > 0
                              ? `${comparativoMensal[1].novas >= comparativoMensal[0].novas ? '+' : ''}${Math.round(((comparativoMensal[1].novas - comparativoMensal[0].novas) / comparativoMensal[0].novas) * 100)}%`
                              : comparativoMensal[1].novas > 0 ? '+100%' : '0%'}
                          </p>
                        </div>
                        <div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-200">
                        <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase">Localizador</th>
                        <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase">Hospede</th>
                        <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase">Hotel</th>
                        <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase">Check-in</th>
                        <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase">Check-out</th>
                        <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase">Valor</th>
                        <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase">Pagamento</th>
                    ...
                    <tbody>
                      {filtered.slice(0, 10).map(b => (
                        <tr key={b.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                          <td className="py-3 px-4 font-mono font-bold text-xs text-[#2e6299]">{b.localizador}</td>
                          <td className="py-3 px-4 text-xs text-slate-600">{b.user_name || '-'}</td>
                          <td className="py-3 px-4 text-xs text-slate-600 max-w-[180px] truncate">{b.hotel_name || '-'}</td>
                          <td className="py-3 px-4 text-xs text-slate-500">{b.check_in ? format(new Date(b.check_in), 'dd/MM/yyyy') : '-'}</td>
                          <td className="py-3 px-4 text-xs text-slate-500">{b.check_out ? format(new Date(b.check_out), 'dd/MM/yyyy') : '-'}</td>
                          <td className="py-3 px-4 text-xs font-semibold text-slate-700">{formatCurrency(b.total_price)}</td>
                          <td className="py-3 px-4">
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                              b.payment_status === 'paid' ? 'bg-emerald-50 text-emerald-600' :
                              b.payment_status === 'pending' ? 'bg-amber-50 text-amber-600' :
                              b.payment_status === 'canceled' ? 'bg-red-50 text-red-600' :
                              'bg-slate-50 text-slate-400'
                            }`}>
                              {PAYMENT_STATUS_LABELS[b.payment_status] || 'Sem pgto'}
                              {b.payment_method && ` · ${PAYMENT_METHOD_LABELS[b.payment_method] || b.payment_method}`}
                            </span>
                          </td>
                          <td className="py-3 px-4">
      ...
      {showMapModal && <MapModal data={stateMapData} onClose={() => setShowMapModal(false)} />}
    </div>
  );
}