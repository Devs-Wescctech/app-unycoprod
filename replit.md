...
- **SerpAPI (Google Hotels):** Preços de mercado para 3 superfícies da LP (carrossel "Melhores Oportunidades", "Comparativo de Preços" e card verde da busca). **Cache mensal (30 dias)** para reduzir consumo (~99%): `/api/lp/market-prices` persiste o resultado completo em `system_config` key `market_prices_cache` (blob 30d) e grava snapshots por cidade em `market_price_snapshots` (city/month/median/max/count); o card verde (`/api/lp/serp-prices`) reaproveita esses snapshots sem chamar a API. Chaves de cidade normalizadas (sem acento, lowercase) via `normalizeCity()` para casar escrita (carrossel) e leitura (card verde). Referência de mercado usa **median** (não max) — labels dizem "preço médio"; usar max inflava a % e gerava falsas ofertas. As 4 superfícies (card verde, overlay do carrossel, badge do HotelCard, comparativo) usam a mesma base median. Chave da API só via `process.env.SERP_API_KEY`. Refresh manual: `POST /api/admin/market-prices/refresh`.
