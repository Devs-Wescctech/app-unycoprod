...
- **Automação E-mail (SMTP):** Página `src/pages/EmailFlows.jsx` (menu Automação > E-mail) espelha a de WhatsApp com tema azul e ícone Mail. Mesmo VisualFlowBuilder (Trigger → Delay → E-mail), com **assunto a nível de fluxo** e toggle **HTML/Texto** (corpo padrão HTML, renderizado via preview). Envio via **nodemailer** (`nodemailer.createTransport`). `triggerEmailFlow` é chamado lado a lado com `triggerWhatsAppFlow` nos mesmos eventos (`booking_confirmed`, `booking_cancelled`, `registration_completed`), usando o e-mail do usuário. Endpoints `/api/email/*` (config, flows CRUD, test, logs, stats). Config SMTP (host/port/secure/user/pass/from_name/from_email) editável via botão "Configurar E-mail", persistida em `system_config` (key: `smtp_config`), senha mascarada (`••••`) no GET e preservada no PUT; transporter cacheado e invalidado ao salvar config. Em produção, cada fluxo envia 1 e-mail (assunto + corpo) com delay; múltiplos nós de mensagem só no modo teste. Permissão de acesso reaproveita `permission: 'whatsapp'`.
...
- **Tarifas por Categoria:** O endpoint `/api/lp/hotels` busca a categoria de cada hotel via InfoHotels (com cache de 30min), aplica a tarifa configurada (alta/baixa temporada) e retorna os preços já ajustados nos cards. A API Coobmais retorna categoria como string ("Silver", "Gold", "Diamante"), o lookup usa `category_name` para match. **Todas as categorias (incluindo Diamante)** seguem o padrão de alta/baixa temporada — cada uma com `low_season_rate` e `high_season_rate` editáveis separadamente no admin (Configurações), aplicados conforme `season_config`.
...
- **Coobmais API:** Para busca de hotéis, reservas, pagamentos e dados de associados. Auto-login via `AccessKey + password` no endpoint `https://apiprod.coobmais.com.br/auth/api/Users/Authenticate` — JWT gerado/cacheado pelo backend (`ensureCoobToken()` em `server/index.js`) com refresh automático 5min antes do `exp`. Configs (`COOBMAIS_AUTH_URL`, `COOBMAIS_ACCESS_KEY`, `COOBMAIS_PASSWORD`) editáveis via Central de APIs ou env. Endpoints de gerenciamento: `GET /api/central/coobmais/token` (preview+exp), `POST /api/central/coobmais/refresh-token` (força regeneração). **Cancelamento de reserva:** o endpoint `/Book/CancellationBook` exige adicionalmente a **senha do associado institucional** no campo `senha` do payload (separada da credencial de integração/JWT). Configurável via `COOBMAIS_CANCEL_PASSWORD` (env) ou Central de APIs (campo "Senha do Associado (cancelamento)" na seção Coobmais), persistida em `system_config` key `api_config` como `Coobmais.cancelPassword`, mascarada no GET e preservada no PUT quando vier mascarada/vazia. Se não configurada, o cancelamento retorna mensagem clara orientando a configuração (sem repassar "Senha não confere").
...
- **SMTP (nodemailer):** Para automação e envio de e-mails. Configurável via UI (Automação > E-mail > "Configurar E-mail") ou env (`SMTP_HOST`, `SMTP_PORT`, `SMTP_SECURE`, `SMTP_USER`, `SMTP_PASS`, `SMTP_FROM_NAME`, `SMTP_FROM_EMAIL`); configs da UI (em `system_config` key `smtp_config`) têm prioridade.
- **SerpAPI (Google Hotels):** Preços de mercado para 3 superfícies da LP (carrossel "Melhores Oportunidades", "Comparativo de Preços" e card verde da busca). **Cache trimestral (90 dias)** para reduzir consumo (~99%): `/api/lp/market-prices` persiste o resultado completo em `system_config` key `market_prices_cache` (blob 90d) e grava snapshots por cidade em `market_price_snapshots` (city/month/median/max/count); o card verde (`/api/lp/serp-prices`) reaproveita esses snapshots sem chamar a API. Chaves de cidade normalizadas (sem acento, lowercase) via `normalizeCity()` para casar escrita (carrossel) e leitura (card verde). Referência de mercado usa **median** (não max) — labels dizem "preço médio"; usar max inflava a % e gerava falsas ofertas. As 4 superfícies (card verde, overlay do carrossel, badge do HotelCard, comparativo) usam a mesma base median. Chave da API só via `process.env.SERP_API_KEY`. Refresh manual: `POST /api/admin/market-prices/refresh`.
  - **Fontes por OTA no Comparativo:** a busca por **cidade** (`engine=google_hotels&q=...`) só devolve um preço agregado por hotel (sem detalhar OTAs). Para listar Booking/Expedia/trivago/etc., o comparativo faz **1 chamada extra por hotel a nível de propriedade** (`q` + `property_token`, via `fetchSerpPropertyPrices`), escolhendo a propriedade mais próxima do `marketPrice` como representativa. As fontes (`sourcePrices`, máx. 5, só as mais caras que a tarifa Unyco) ficam embutidas no blob 90d, então o custo extra só ocorre no recálculo completo.
  - **Limpar caches:** `POST /api/admin/caches/clear` (botão "Limpar caches" em Configurações) limpa os caches em memória **e** remove do banco o blob `market_prices_cache` e todos os `market_price_snapshots` — sem isso, o blob de 90 dias continuaria sendo servido e o Comparativo não resetaria.
